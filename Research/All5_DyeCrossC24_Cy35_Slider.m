%% 固定总浓度，改变Cy3占比（控制调变按钮，未完成）
%%
function All5_DyeCrossC24_Cy35_Slider
clear; clc;
...
NameSet = struct('Cy', {{'Cy3', 'Cy5'}}, 'ShowExp', {{'Show Exp', 'Show Exp'}});
% 删除Exp实验点的代码还没写好
...
HC = uicontrol(...
    ...
    'callback', @(s, e) @sliderCallback);
uif = uifigure;
set(uif, 'Position', [1600, 400, 160, 160]); 
BC = uibutton(uif, ...
    "Text",NameSet.Cy{NC}, ...
    "Position",[40 60 80 80],...
    "ButtonPushedFcn", @(src,event) buttonC);
BE = uibutton(uif, ...
    "Text",NameSet.ShowExp{NE+1}, ...
    "Position",[40 20 80 30],...
    "ButtonPushedFcn", @(src,event) buttonE);

Ep = [];
for row=1:7
    for col=1:5
        Ep(row,col) = plot(0,0,'r+ '); hold on;
    end
end
function buttonC
    NC = mod(NC, 2)+1;
    BC.Text = NameSet.Cy{NC};
    buttonAct;
end
function buttonE
    NE = mod(NE+1, 2);
    BE.Text = NameSet.ShowExp{NE+1};
    buttonAct;
end
function buttonAct
    for i=2:6
    ...
    if NE == 1
        ...
        for i=2:6
            for j=1:length(Exp(:,1))
                ev = Exp(j,i);
                if ev >= 0
                    Ep(j,i-1) = plot(Exp(j,1),ev,'+','color',ColorSet(i-1,:)); hold on;
                end
            end
        end
    end
